...
//I	0.00	tiger_palm,if=talent.chi_explosion.enabled&(cooldown.fists_of_fury.remains<5|cooldown.fists_of_fury.up)&buff.tiger_power.remains<5
            await Spell.Cast(S.TigerPalm, onunit, () => (!TalentManager.IsSelected(20) && Me.GetAuraTimeLeft("Tiger Power").TotalSeconds <= 6.6) || (TalentManager.IsSelected(20) && (Spell.GetCooldownLeft("Fist of Fury").TotalSeconds < 5 || !SpellManager.Spells["Fists of Fury"].Cooldown) && Me.GetAuraTimeLeft("Tiger Power").TotalSeconds < 5) || !Me.HasAura("Tiger Power"));
...
//O	1.95	rising_sun_kick,if=(debuff.rising_sun_kick.down|debuff.rising_sun_kick.remains<3)
            await Spell.Cast(S.RisingSunKick, onunit, () => Me.CurrentTarget.HasAuraExpired("Rising Sun Kick", 3) || !Me.CurrentTarget.HasAura("Rising Sun Kick", true));
...
//Q	15.14	fists_of_fury,if=buff.tiger_power.remains>cast_time&debuff.rising_sun_kick.remains>cast_time&energy.time_to_max>cast_time&!buff.serenity.up
            await Spell.Cast(S.FistsofFury, onunit, () => Me.GetAuraTimeLeft("Tiger Power") >= Spell.GetSpellCastTime("Fist of Fury") && Me.CurrentTarget.GetAuraTimeLeft("Rising Sun Kick") >= Spell.GetSpellCastTime("Fist of Fury") && Me.CurrentEnergy + EnergyRegen < 80 && !Me.HasAura(S.Serenity));
...
//U	6.15	energizing_brew,if=cooldown.fists_of_fury.remains>6&(!talent.serenity.enabled|(!buff.serenity.remains&cooldown.serenity.remains>4))&energy+energy.regen<50
            ...
            await st(onunit, Units.EnemyUnitsSub8.Count() < 3 && !TalentManager.IsSelected(20));
...
//Y	0.00	call_action_list,name=aoe_norjw,if=active_enemies>=3&!talent.rushing_jade_wind.enabled&!talent.chi_explosion.enabled
            await AOEnorjw(onunit, Units.EnemyUnitsSub8.Count() >= 3 && !TalentManager.IsSelected(16) && !TalentManager.IsSelected(20) && Axiom.AOE);
...
//a	0.00	call_action_list,name=aoe_rjw,if=active_enemies>=3&talent.rushing_jade_wind.enabled
        ...
        #region Coroutine ST
        private static async Task<bool> st(WoWUnit onunit, bool reqs)
        {
            if (!reqs) return false;
//actions.st=rising_sun_kick
            await Spell.Cast(S.RisingSunKick, onunit);
...
//actions.st+=/jab,if=chi.max-chi>=2
            await Spell.Cast(S.Jab, onunit, () => Me.ChiInfo.Max - Me.CurrentChi >= 2);
...
//actions.aoe+=/rising_sun_kick,if=chi=chi.max
            await Spell.Cast(S.RisingSunKick, onunit, () => Me.CurrentChi == Me.MaxChi);
...
//actions.aoe_rjw+=/jab,if=chi.max-chi>=2
        ...
        private static async Task<bool> Aoerjw(WoWUnit onunit, bool reqs)
        {
            if (!reqs) return false;
            //actions.aoe_rjw+=/rushing_jade_wind
            await Spell.Cast(S.RushingJadeWind, onunit);
