...
namespace Axiom.Managers
{
    class TalentManager
    {
        ...
        public static bool IsSelected(int index)
        {
            ...
            int tier = (index - 1) / 3;
            if (tier.IsBetween(0, 5))
            {
                Log.WritetoFile(Styx.Common.LogLevel.Diagnostic, string.Format("Talent id {0} is selected", index));
        ...
        private static void UpdateTalentManager(object sender, LuaEventArgs args)
        {
            ...
            Log.WritetoFile(Styx.Common.LogLevel.Diagnostic, string.Format("{0} Event Fired!", args.EventName));
            ...
            if (args.EventName == "PLAYER_SPECIALIZATION_CHANGED")
            {
                ...
                Log.WritetoFile(Styx.Common.LogLevel.Diagnostic, string.Format("TalentManager: receive a {0} event, currently {1} -- queueing check for new spec!", args.EventName, CurrentSpec));
            }
